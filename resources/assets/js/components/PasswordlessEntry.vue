			...
			<div class="field">
				<div class="columns">
					<div class="column is-6">
						<p class="control">
							<button class="button is-small" :class="[showIsLoading ? ' is-loading' : '', ' is-primary']" @click.prevent="sendMagicLink">
							<span class="icon is-small is-left"><i class="fa fa-envelope"></i></span>&nbsp;&nbsp;&nbsp;Send magic link
							</button>
						</p>
					</div>
					<div class="column is-6">
						<p><a href="/login">Login in with password instead &rarr;</a></p>
					</div>
				</div>
			</div>
    	</div>
			...
			sendMagicLink() {
				...
				axios.post('/passwordless-entry', {
					...
					.catch(({response}) => {
						...
						if (!formErrors.email) {
							this.showNotice('Notice', 'Sorry, a problem occured while sending magic link', 'danger');
							this.email = null;
						}
					});
